...
try:
    ...
    subprocess.run(["pip", "install", "-q", "torch", "torchaudio", "huggingface_hub"], check=True)
    subprocess.run(["git", "clone", "https://github.com/alibaba-damo-academy/FunASR.git"], check=True)
    subprocess.run(["cd", "FunASR", "&&", "pip", "install", "-e", "."], check=True, shell=True)
    
    # Clone the CSM repository if it doesn't exist
    if not os.path.exists("conversational-speech-model"):
        subprocess.run(["git", "clone", "https://github.com/facebookresearch/conversational-speech-model.git"], check=True)
        subprocess.run(["cd", "conversational-speech-model", "&&", "pip", "install", "-e", "."], check=True, shell=True)
        sys.path.append("conversational-speech-model")
    else:
        sys.path.append("conversational-speech-model")
except ImportError:
    IN_COLAB = False
...
# 구글 코랩에서 사용하는 방법:
# 1. 구글 코랩에서 새 노트북을 만드세요
# 2. 이 스크립트 전체를 셀에 복사하세요
# 3. 실행 시 Hugging Face 토큰을 입력하세요 (https://huggingface.co/settings/tokens 에서 토큰 생성 가능)
# 4. 스크립트가 실행되어 음성을 생성합니다
# 5. 생성된 파일은 자동으로 다운로드됩니다
#
# 원하는 텍스트로 변경하려면:
